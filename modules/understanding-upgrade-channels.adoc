...
Beginning with {product-title} 4.10 whenever update recommendations are removed from supported channels they are replaced with Conditional Updates which declare one or more known risks. Each known risk may apply to all clusters or only clusters matching certain conditions such as having `Platform` set to `None` or the CNI provider set to `OpenShiftSDN`. The Cluster Version Operator (CVO) continually evaluates known risks against the current cluster state. When no risks match, the update is recommended. If the risk matches those updates are listed as Supported But Not Recommended Updates, a reference link is provided which helps the cluster admin decide if they would like to accept the risk and update anyway.
...
Additionally, there are several factors which may lead an organization to move clusters to the fast channel either permanently or temporarilly including:
 ...
 - Application of CVE fixes without delay -- even CVE fixes may introduce regressions, so we still apply promotion delays to z-streams with CVE fixes.
 - Internal testing processes -- If it takes your organization several weeks to qualify releases it's best test concurrently with our promotion process rather than waiting. This also assures that any telemetry signal provided to Red Hat is a factored into our rollout, we can fix issues relevant to you faster.
