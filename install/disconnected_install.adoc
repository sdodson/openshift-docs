...
== Installing and configuring a stand alone registry

To avoid copying all the Docker images to each node host, install a
standalone registry on your disconnected LAN to serve the images to your nodes.

. On the webserver within your disconnected LAN,
xref:../install/stand_alone_registry.adoc#install-config-installing-stand-alone-registry[install a stand alone registry].
+
Note the domain name of your registry.

. Create a namespace to store your images in your registry:
+
----
$ <add command>
----
...
<update list with more tar files>
----

. Tag the infrastructure component images:
+
[source, bash]
----
$ docker tag registry.access.redhat.com/rhel7/etcd:3.2.22 registry.example.com/rhel7/etcd:3.2.22
<add the rest of the images for this section>
----

. Push the infrastructure component images:
+
[source, bash]
----
$ docker push registry.example.com/rhel7/etcd:3.2.22
<add the rest of the images for this section>
----

. <add commands to tag and push images from the other sections>
